...
const Navbar = ({ theme, toggleTheme }) => {
  ...
  return (
    <nav className={`navbar shadow-lg ${theme === 'light' ? 'bg-white text-black' : 'bg-gray-800 text-white'} transition-colors duration-300`}>
      <div className="container mx-auto flex justify-between items-center p-4">
        ...
        {/* Theme Toggle Button */}
        <button onClick={toggleTheme} className="ml-4 bg-slate-100 toggle-theme" aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}>
          {theme === 'light' ? '🌙' : '☀️'}
        </button>
